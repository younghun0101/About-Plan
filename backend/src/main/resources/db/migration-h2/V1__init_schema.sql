-- About Plan initial schema (H2 local profile)
...
create table if not exists users (
    ...
    dte_created_at       timestamp with time zone not null default current_timestamp,
    dte_updated_at       timestamp with time zone not null default current_timestamp
);
...
create table if not exists events (
    ...
    dte_start_at             timestamp with time zone not null,
    dte_end_at               timestamp with time zone not null,
    ...
    dte_deleted_at           timestamp with time zone null,
    dte_created_at           timestamp with time zone not null default current_timestamp,
    dte_updated_at           timestamp with time zone not null default current_timestamp,
...
create table if not exists goals (
    ...
    dte_deadline             timestamp with time zone not null,
...
merge into users key(tbl_user_id) values
    ('user-a-001', 'User A', 'usera@aboutplan.com', 'password123', current_timestamp, current_timestamp),
    ('user-b-002', 'User B', 'userb@aboutplan.com', 'password123', current_timestamp, current_timestamp);

merge into shared_calendars key(tbl_shared_calendar_id) values
    ('shared-cal-001', '공동 캘린더', 'user-a-001', current_timestamp, current_timestamp);
