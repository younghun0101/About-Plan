...
import { Target, Plus, Pencil, Trash2, ChevronDown, Calendar } from 'lucide-react'
...
export default function PersonalGoalsPage() {
  ...
  const pendingGoals = personalGoals.filter(g => !g.bln_is_completed)
  const completedGoals = personalGoals.filter(g => g.bln_is_completed)
  ...
  return (
    <AppShell title="개인 목표">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-foreground">개인 목표</h2>
            <p className="text-muted-foreground">목표를 설정하면 캘린더에 자동으로 표시됩니다.</p>
          </div>
        ...
        {personalGoals.length === 0 ? (
          <Empty
            ...
            title="목표를 설정하면 캘린더에 자동으로 표시됩니다"
            description="첫 번째 목표를 추가해보세요."
            action={{
              label: '목표 추가',
        ...
        <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          <DialogContent className="sm:max-w-[480px]">
            <DialogHeader>
              <DialogTitle>{selectedGoal ? '목표 수정' : '새 목표'}</DialogTitle>
              <DialogDescription>
                목표와 데드라인을 설정하세요. 캘린더에 자동으로 표시됩니다.
              </DialogDescription>
            </DialogHeader>
